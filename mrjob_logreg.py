...
class MRLogReg(MRJob):
	...
	def mapper(self, key, line):
		...
		for vars in line:
			...
			for var in varlist:
				if cont_dict[var] == 0:
					levels = data2[var].unique()
					i = 0		
					for l in levels[1:]:
						data2[var + str(i)] = 1 * (np.array(data2[var]) == l)
						data2_t[var + str(i)] = 1 * (np.array(data2_t[var]) == l)
			...
			try:
				...
				t1err = len(filter(lambda x: x == -1, errs)) / float(len(np.array(data2['CLASS'])) - sum(np.array(data2['CLASS'])))
				t2err = len(filter(lambda x: x == 1, errs)) / float(sum(np.array(data2['CLASS'])))


				preds_test = fit.predict(data2_t[regvars])
				errs_test = np.array(data2_t['CLASS']) - np.round(preds_test)

				t1err_t = len(filter(lambda x: x == -1, errs_test)) / float(len(np.array(data2_t['CLASS'])) - sum(np.array(data2_t['CLASS'])))
				t2err_t = len(filter(lambda x: x == 1, errs_test)) / float(sum(np.array(data2_t['CLASS'])))
